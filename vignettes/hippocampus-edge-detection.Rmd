...
The **SpatialDryArtifacts** R package provides a robust, two-step workflow to identify, classify, and handle edge dryspot artifacts in spatial transcriptomics data, particularly from the **10x Visium** platform.  
These artifacts, often appearing as areas of low gene and UMI counts at tissue edges, can negatively impact downstream analyses.
...
- **Morphological Cleaning**: With helper functions (`focal_transformations`), performs image-analysis-style operations to fill gaps and connect nearby outliers into contiguous regions.  
- **Cluster Detection**: Groups these outliers into contiguous “problem areas” (`problemAreas`).  
- **Edge Identification**: Checks which clusters touch the tissue border (`clumpEdges`).  
...
**Goal**: Take the raw detections from Step 1 and apply user-defined rules to assign actionable QC labels.  

**How does it work**:

- **Input**: Requires the `spe` object processed by `detectEdgeDryspots()`.  
- **Rule-Based Classification**: Evaluates clusters with user parameters such as `min_spots` and `removal_threshold`.  
- **Label Assignment**: Assigns human-readable QC labels like `"remove"`.  
- **Key Outputs**: Adds final classification columns: `_classification` and `_enhanced`.

---

## Parameter Guide

Tuning parameters lets you adapt the workflow to different tissue types and data quality.
...
- *`mad_threshold`* (Default: 3) – Sensitivity for detecting outliers. Lower values (1.5–2) are more sensitive.  
- *`edge_threshold`* (Default: 0.75) – Proportion of boundary openness required to classify as edge. Lower values are more aggressive.  
- *`min_cluster_size`* (Default: 40) – Minimum cluster size. Lower values detect smaller artifacts.

**Advanced & Data Structure Parameters**

- *`qc_metric`* (Default: `"sum_gene"`) – Column used for outlier detection (`"sum_umi"` also possible).  
- *`samples`* (Default: `"sample_id"`) – Column for sample IDs; processed independently.  
- *`batch_var`* (Default: `"both"`) – Determines grouping for MAD calculation. Options: `"sample_id"`, `"slide"`, or `"both"`.  
- *`shifted`* (Default: FALSE) – Set TRUE for Visium odd-column shift correction.

**Utility Parameters**

- *`name`* (Default: `"edge_dryspot"`) – Prefix for output columns.  
- *`verbose`* (Default: TRUE) – Print progress messages.  
- *`keep_intermediate`* (Default: FALSE) – Retain intermediate outlier columns.

---

### **`classifyEdgeDryspots()` Parameters**

- *`min_spots`* (Default: 20) – Minimum cluster size to classify as `"remove"` or `"flag"`.  
- *`low_umi_threshold`* (Default: 100) – UMI cutoff for low-quality spots.  
- *`removal_threshold`* (Default: 0.5) – Proportion of low-UMI spots required to classify cluster as `"remove"`.  
- *`exclude_slides`* (Default: NULL) – Vector of slides to ignore edge calls.  
- *`name`* (Default: `"edge_dryspot"`) – Must match name from `detectEdgeDryspots()`.
...
- **`*_classification`** – Final classification: `"remove"`, `"flag"`, `"small"`, `"keep"`.  
- **`*_enhanced`** – Final classification: More detailed context.  
- **`*_true_edges`** – Intermediate: Edge status after applying `exclude_slides`.
